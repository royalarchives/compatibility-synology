...
module.exports = {
  wantsRequest: async (catalog, req) => {
  ...
  handleRequest: async (catalog, req, res) => {
    ...
    if (req.sourcePath) {
      try {
        ...
        await executeRoute(catalog, req, res, req.sourcePath)
        return
      } catch (error) {
...
async function executeRoute (catalog, req, res, filePath) {
  try {
    const sourceFile = require(filePath)
    return sourceFile(catalog, req, res)
  } catch (error) {
